    ...
    for bucket_object in bucket_ob.objects.all():
        ...
        if str_path not in resume_links:
            s3_client.download_file(
                Bucket="bucketofresume", Key=bucket_object.key, Filename=bucket_object.key
            )
...
def insertParsedResumeData(resume_link, data, connection, cursor):
    ...
    sql = "UPDATE tbl_resume_link SET Loaded_flag = 1 WHERE Resume_link = %s"
    record = (resume_link,)
    cursor.execute(sql, record)
...
def getShortlistedCandidates(rolename, cursor):
    query = """SELECT ID, Role_name, Candidate_Name, Contact_Email, Contact_phone, Work_Experience, Skill_set, Education, candidate_score  FROM tbl_candidate_resume cr INNER JOIN tbl_shortlisted_candidates sc ON sc.candidate_ID = cr.ID WHERE sc.Role_name = %s"""
    role_name = [rolename]
...
def getRelevantInterviewers(cursor, role_name):
    ...
    getInterviewers(skillset, sql_exp, cursor)


def submitFeedback(cursor, connection, c_name, c_feedback):
    sql = "INSERT INTO tbl_feedback (Candidate_Name, Feedback) VALUES (%s, %s)"
    val = (c_name, c_feedback)
    cursor.execute(sql, val)
...
def main():
    ...
    # get relevant interviewers for the open role
    relevant_interviewers = getRelevantInterviewers(cursor, "Support Engineer")
    print(relevant_interviewers)

    # Submit feedback for a candidate
    c_name = "Dhanasree Rajamani"
    c_feedback = "Good"
    submitFeedback(cursor, connection, c_name, c_feedback)
